...
class AIClient:
    ...
    def request(self, messages, model_alias=None, max_tokens=None):
        ...
        if model_alias not in self.config.get('models', {}):
            raise ValueError(
                f"Model alias '{model_alias}' not found in configuration")
        ...
        if not provider:
            raise ValueError(
                f"Provider not specified for model alias '{model_alias}'")

        print(
            f"Requesting content from model '{model_alias}' using provider '{provider}'")
    ...
    def _azure_openai_request(self, messages, model_config, max_tokens):
        ...
        if 'model_name' not in model_config or not model_config['model_name']:
            raise ValueError(
                "Azure deployment name not provided for Azure OpenAI, please set it as 'model_name' in the configuration")
